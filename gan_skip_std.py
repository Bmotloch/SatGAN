...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            ...
            nn.LeakyReLU(0.2, inplace=True),
            nn.utils.spectral_norm(nn.Conv2d(1024, 1024, kernel_size=4, stride=2, padding=1)),
            nn.LeakyReLU(0.2, inplace=True),
            nn.utils.spectral_norm(nn.Conv2d(1024, 1, kernel_size=4, stride=1, padding=0)),
            nn.Flatten()
        )
...
class StitchDataset(Dataset):
    def __init__(self, input_dir, stitched_dir, mode='row'):
...
def train_gan(generator, discriminator, dataloader, num_epochs=10, device='cuda', last_epoch=0, save_dir="checkpoints",
    ...
    optimizer_G = optim.Adam(generator.parameters(), lr=0.00005, betas=(0.75, 0.999))
    optimizer_D = optim.Adam(discriminator.parameters(), lr=0.00005, betas=(0.75, 0.999))
...
if __name__ == "__main__":
    ...
    dataset = StitchDataset(input_dir, stitched_dir, mode=mode)
    ...
    train_gan(generator, discriminator, dataloader, num_epochs=30, device='cuda', last_epoch=last_epoch,
              save_dir=save_dir, log_file="training_log.csv")
