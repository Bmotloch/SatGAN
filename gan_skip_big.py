...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.encoder1 = nn.Sequential(
            nn.Conv2d(7, 96, kernel_size=4, stride=2, padding=1),
        ...
        self.middle = nn.Sequential(
            nn.Conv2d(768, 768, kernel_size=4, stride=2, padding=1),
            nn.InstanceNorm2d(768),
            nn.ReLU(inplace=True),
        )

        self.decoder4 = nn.Sequential(
            nn.ConvTranspose2d(768, 768, kernel_size=4, stride=2, padding=1),
            nn.InstanceNorm2d(768),
            nn.ReLU(inplace=True),
        )
        self.decoder3 = nn.Sequential(
            nn.ConvTranspose2d(1536, 384, kernel_size=4, stride=2, padding=1),
            nn.InstanceNorm2d(384),
            nn.ReLU(inplace=True)
        )
        self.decoder2 = nn.Sequential(
            nn.ConvTranspose2d(768, 192, kernel_size=4, stride=2, padding=1),
            nn.InstanceNorm2d(192),
            nn.ReLU(inplace=True)
        )
        ...
        self.final = nn.Sequential(
            nn.ConvTranspose2d(192, 3, kernel_size=4, stride=2, padding=1),
            nn.Tanh()
        )
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            nn.utils.spectral_norm(nn.Conv2d(3, 96, kernel_size=4, stride=2, padding=1)),
            nn.LeakyReLU(0.2, inplace=True),
            nn.utils.spectral_norm(nn.Conv2d(96, 192, kernel_size=4, stride=2, padding=1)),
            nn.LeakyReLU(0.2, inplace=True),
            nn.utils.spectral_norm(nn.Conv2d(192, 384, kernel_size=4, stride=2, padding=1)),
            nn.LeakyReLU(0.2, inplace=True),
            nn.utils.spectral_norm(nn.Conv2d(384, 768, kernel_size=4, stride=2, padding=1)),
            nn.LeakyReLU(0.2, inplace=True),
            nn.utils.spectral_norm(nn.Conv2d(768, 1, kernel_size=4, stride=1, padding=0)),
            nn.Flatten(),
        )
...
if __name__ == "__main__":
    ...
    train_gan(generator, discriminator, dataloader, num_epochs=30, device=device, last_epoch=last_epoch,
              save_dir=save_dir, log_file="training_log_big.csv")
